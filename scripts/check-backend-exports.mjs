...
const scopedPaths = [path.join(rootDir, 'apps/api/src')];
...
console.log('🔍 Checking for default exports in backend source paths...\n');
...
if (hasError) {
  console.log('❌ Contract gate FAILED: Default exports found in backend source paths');
  console.log('\nAllowed exceptions: *.test.ts, __tests__/**, *.generated.ts, *.d.ts');
  process.exit(1);
} else {
  console.log('✅ Contract gate PASSED: No default exports in backend source paths');
  process.exit(0);
}
