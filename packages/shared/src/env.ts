...
export const backendEnvSchema = z
  .object({
    ...
    JWT_PRIVATE_KEY_ENCRYPTION_KEY: z
      ...
      .default('dev-test-encryption-key-32-chars!'),
    ...
    FEATURE_GATE_JOIN_GUILDS: z.coerce.number().int().nonnegative().default(100),
    FEATURE_GATE_CREATE_GUILD: z.coerce.number().int().nonnegative().default(500),
    FEATURE_GATE_COMPETITIVE_RANKED: z.coerce.number().int().nonnegative().default(800),
    FEATURE_GATE_MODERATE_FORUMS: z.coerce.number().int().nonnegative().default(600),
  ...
  .refine(
    (config) => {
      if (config.NODE_ENV !== 'production') return true;
      if (config.STRIPE_WEBHOOK_SECRET === undefined) return true;
      if (config.STRIPE_WEBHOOK_SECRET === '') return true;
      return config.STRIPE_WEBHOOK_SECRET.startsWith('whsec_');
    },
    {
      message:
