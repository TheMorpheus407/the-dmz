...
describe('MFA WebAuthn Challenge Store (Redis)', () => {
  ...
  describe('Challenge storage with Redis', () => {
    ...
    it('uses correct key prefix for challenge storage', () => {
      const challengeId = 'test-challenge-123';
      const expectedKey = `mfa:webauthn:challenge:${challengeId}`;
      expect(expectedKey).toBe('mfa:webauthn:challenge:test-challenge-123');
    });
    ...
    it('stores and retrieves challenge via Redis mock', async () => {
      ...
      await mockRedisClient.setValue(`mfa:webauthn:challenge:${challengeId}`, serialized, 300);

      const retrieved = await mockRedisClient.getValue(`mfa:webauthn:challenge:${challengeId}`);

      expect(retrieved).toBe(serialized);
      expect(mockRedisClient.setValue).toHaveBeenCalledWith(
        'mfa:webauthn:challenge:challenge-123',
        expect.any(String),
        300,
      );
    });
    ...
    it('deletes challenge after use', async () => {
      ...
      await mockRedisClient.deleteKey(`mfa:webauthn:challenge:${challengeId}`);

      expect(mockRedisClient.deleteKey).toHaveBeenCalledWith(
        'mfa:webauthn:challenge:challenge-to-delete',
      );
    });
    ...
    it('returns null for non-existent challenge', async () => {
      ...
      const result = await mockRedisClient.getValue('mfa:webauthn:challenge:non-existent');
  ...
  describe('Multi-instance deployment compatibility', () => {
    it('challenge can be stored and retrieved across different "instances"', async () => {
      ...
      await redisInstanceA.setValue(`mfa:webauthn:challenge:${challengeId}`, serialized, 300);

      const retrievedByInstanceB = await redisInstanceB.getValue(
        `mfa:webauthn:challenge:${challengeId}`,
      );

      expect(retrievedByInstanceB).not.toBeNull();
      expect(retrievedByInstanceB).toBe(serialized);

      await redisInstanceB.deleteKey(`mfa:webauthn:challenge:${challengeId}`);

      redisInstanceA.getValue.mockResolvedValueOnce(null);

      const afterDelete = await redisInstanceA.getValue(`mfa:webauthn:challenge:${challengeId}`);
