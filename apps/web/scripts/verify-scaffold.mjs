...
const eventsSource = fs.readFileSync(path.join(root, 'src/lib/game/state/events.ts'), 'utf8');
if (!/export\s+interface\s+GameEvent/.test(eventsSource)) {
  throw new Error('Expected GameEvent interface stub in events.ts');
}

const reducerSource = fs.readFileSync(path.join(root, 'src/lib/game/state/reducer.ts'), 'utf8');
